...
| 状态分组规则、CSV 列契约 | `data/schema.js` |
| 颜色 | `data/palette.js` |
| 版本号 / 数据路径 | `data/config.js` |
...
- `DATA_VERSION`（`data/config.js`）——**数据**时钟。它同时是「本次更新」高亮标记
  （`item.updated === DATA_VERSION`）。**只在数据真的变了时才 bump**；为纯代码
  改动 bump 会静默清空所有更新标记。必须与 `src/reports/index.js` 里的 `date` 一致。
- `REPORT_VERSION`（`src/reports/2026-04-global-lithium/index.jsx`）——**代码**时钟。
  必须与 `index.html` 里 `app.js?v=` 一致。

四处不一致会让 `npm run build` 失败（见下）。
